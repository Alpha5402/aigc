...
const buildFallbackMarketReport = ({ crop, region, question, results }) => {
  ...
  return `【行情概况】
${hasData ? `数据不足，仅作参考：已检索到 ${results.length} 条可参考的公开行情资料，但直接支撑“${crop}”精确价格判断的信息仍有限。` : `数据不足，仅作参考：当前可用公开行情资料不足以支撑对“${crop}”的精确价格判断。`}${
    region ? `地区关注点为${region}。` : ''
  }

【价格变化】
${evidenceText || (hasData ? '已检索到可参考资料，但未抽取到足够清晰的作物价格句；兜底报告不新增编造价格。' : '暂未检索到足够的直接成交价、价格指数或涨跌幅数据。')}
...
【风险提示】
本报告为资料不足时的兜底分析，不替代真实成交价。最终出货决策应以当地市场实时询价、采购合同和运输损耗核算为准。
