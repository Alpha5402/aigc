...
async def _run_adapter(family: str, history_values: list[float], missing_mask: list[int], horizon: int) -> ModelOutput:
    ...
    try:
        await asyncio.to_thread(adapter.fit, history_values, missing_mask)
        bundle = await asyncio.to_thread(adapter.predict, horizon)
...
async def _forecast_one(req: ForecastRequest) -> ForecastResponse:
    ...
    async with _INFERENCE_SEM:
        results = await asyncio.gather(
            *(
                _run_adapter(family, history_values, missing_mask, horizon)
